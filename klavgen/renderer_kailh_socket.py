from .config import KailhSocketConfig, Config


def draw_socket_base(wp, cf: KailhSocketConfig):
    return (
        wp.lineTo(cf.front_flat_width, 0)
        .radiusArc(
            (cf.front_flat_width + cf.large_radius, cf.large_radius), radius=-cf.large_radius
        )
        .lineTo(cf.front_flat_width + cf.large_radius, cf.socket_total_depth)
        .lineTo(
            cf.front_flat_width + cf.large_radius - cf.back_right_flat_width, cf.socket_total_depth
        )
        .radiusArc(
            (
...
def draw_socket_left_pin(wp, cf: KailhSocketConfig):
    return (
        wp.moveTo(0, cf.socket_thin_part_depth - cf.left_flat_depth_behind_solder_pin)
        .lineTo(
            -cf.solder_pin_width, cf.socket_thin_part_depth - cf.left_flat_depth_behind_solder_pin
        )
...
def draw_socket_right_pin(wp, cf: KailhSocketConfig):
    return (
        wp.moveTo(
